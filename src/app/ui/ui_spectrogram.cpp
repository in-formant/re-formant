...
void reformant::ui::spectrogram(AppState& appState) {
    ...
    SpectrogramController& spectrogramController =
        *appState.spectrogramController;
    ...
    if (ImGui::Begin("Spectrogram")) {
        ...
        if (ImGui::SliderDouble("##scrub", &sliderTime, 0,
                                appState.audioTrack.duration(), "%.3f s")) {
        ...
        if (ImPlot::BeginSubplots(
            ...
            if (ImPlot::BeginPlot("##spectrogram")) {
                ...
                ImPlot::SetupAxisLinks(ImAxis_Y1, &appState.ui.plotFreqMin,
                                       &appState.ui.plotFreqMax);
                ...
                int yScale = (int)ImPlot::GetAxisScale(ImAxis_Y1);
                if (yScale != appState.ui.plotFreqScale) {
                    appState.settings.setSpectrumFreqScale(yScale);
                    appState.ui.plotFreqScale = yScale;
                }

                const double timePerPixel = ImPlot::PixelsToPlot({1, 0}).x -
                                            ImPlot::PixelsToPlot({0, 0}).x;
                ...
                if (!spectrogram.data.empty()) {
                    ImPlot::PlotHeatmap(
                        "##spectrogram_heatmap", spectrogram.data.data(),
                        spectrogram.numFreqs, spectrogram.numSlices,
                        appState.ui.spectrumMinDb, appState.ui.spectrumMaxDb,
                        nullptr, {spectrogram.timeMin, spectrogram.freqMin},
                        {spectrogram.timeMax, spectrogram.freqMax});
                }

                auto pitches = pitchController.getPitchesForRange(
                    rect.X.Min, rect.X.Max, timePerPixel);

                ImPlot::SetNextMarkerStyle(ImPlotMarker_Circle,
                                           4.0f * appState.ui.scalingFactor,
                                           ImVec4(0.49f, 0.98f, 1.00f, 1.00f),
                                           1.0f * appState.ui.scalingFactor,
                                           ImVec4{0.00f, 0.00f, 0.00f, 0.75f});
                ...
                if (ImPlot::DragLineX(838492, &dragTime, {1, 1, 1, 1}, 2,
                    ...
                    if (dragTime >= 0 &&
                        dragTime <= appState.audioTrack.duration()) {
                ...
                if (ImGui::BeginPopup("Range")) {
                    if (ImGui::SliderDouble("Max", &appState.ui.spectrumMaxDb,
                        ...
                        appState.settings.setSpectrumMaxDb(
                            appState.ui.spectrumMaxDb);
                    }
                    if (ImGui::SliderDouble("Min", &appState.ui.spectrumMinDb,
                                            -100, appState.ui.spectrumMaxDb)) {
                        appState.settings.setSpectrumMinDb(
            ...
            if (ImPlot::BeginPlot("##waveform_plot")) {
                ...
                int stride =
                    std::max(1, (int)std::ceil(pltPerX * sampleRate) / 16);

                ImPlot::PlotLine(
                    "##waveform", envelope.data(), envelope.size() / stride - 1,
                    stride / sampleRate, 0.0, 0, stride * sizeof(float));
        ...
        if (timeCursorChangedForcefully ||
            appState.ui.isInTimeScrollAnimation) {
            ...
            if (newTime < appState.ui.plotTimeMin ||
    ...
    if (appState.audioOutput.isPlaying() &&
        spectrogramController.timeSamples() >=
            appState.audioTrack.sampleCount()) {
        appState.audioOutput.stopPlaying();
    }
}